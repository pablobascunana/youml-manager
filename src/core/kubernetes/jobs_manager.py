...
class KubernetesJobManager:
    ...
    def _create_job_object(job_name: str, image_name: str, params: Dict[str, str]):
        container = client.V1Container(name=job_name, image=image_name, image_pull_policy="Always")  # args=[params["dataset"], params["path"]]
    ...
    def get_job_status(api_instance: BatchV1Api, job_name: str):
        ...
        while not job_completed:
            ...
            if api_response.status.succeeded is not None or api_response.status.failed is not None:
                job_completed = True
            time.sleep(1)
            logger.info(f"Job status: status={api_response.status}")
